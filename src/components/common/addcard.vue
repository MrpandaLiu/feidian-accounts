<template>
    <div class="card" @click="goPerson">
        ...
        <div>
            <span id="email" ref="email">{{datas.email}}</span>
        </div>
        <div>
            <span>{{datas.group}}</span>
        </div>
        <div class="left"  @click.stop="handleAdd">
            <span id="add">同意</span>
        </div>
        <div class="right"  @click.stop="handleRefuse">
            <span id="refuse">拒绝</span>
        </div>
    </div>
</template>
...
export default {
    ...
    methods:{
        ...
        handleAdd() {
            let btn = confirm("请确认是否添加")
            if(btn == true){
                this.$store.dispatch('addMess',this.data.email);
            }
        },
        handleRefuse() {
            let btn = confirm("请确认是否拒绝")
            if(btn == true){
                this.$store.dispatch('RefuseMess',this.data.email);
            }
        },
        smallEmail() {
            if(this.data.email.toString().length >= 20){
                document.getElementById('email').style.fontSize = 12 +'px';
            }
        }
    },
    mounted(){
