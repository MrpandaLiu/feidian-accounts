<template>
    <div class="index">
        <div class="index-introduce" :style="{height}">
            ...
            <div>
                <ul>
                    <a @click="personalCenter"><li>管理员の中心</li></a>
                    <a @click="auditPerson"><li>审核新成员</li></a>
                    <a @click="downloadExcel"><li>输出成员Excel</li></a>
                </ul>
            </div>
           <!--  <div>
                <p>沸点账号中心</p>
            ...
            <div class="person-message">
                ...
                <div class="person-message-content">
                    <div v-for="(data,index) in pageData" :key="index" >
                        <Delcard  v-if="!data.del && data.pass && !data.admin" :data="data" class="card" ></Delcard>
                    </div>
                </div>
            </div>
...
import Delcard from './common/delcard.vue'
...
export default {
    ...
    computed :{
        filterData:function(){
            let arr = [...this.datas];
            arr = arr.filter((item) => item.pass == true && item.del == false);
            if(this.searchCode != ''){
    ...
    methods:{
        ...
        auditPerson(){
            this.$router.push('/audit/'+this.LoginMessage.email);
        },
        downloadExcel(){
